...
static const char * payload  = "{\"procedure\":" "\"" crossbar_call_function "\""",\"args\":[" "\""call_args"\"""]}";
...
{
      printf(" start: %s\n",CROSSBAR_HTTP_BRIDGE_VALUE );
     parsed_url = (url_parser_url_t *) malloc(sizeof(url_parser_url_t));
     printf(" parsed_url = (url_parser_url_t *) malloc(sizeof(url_parser_url_t)): %d\n",sizeof(url_parser_url_t));
    ...
    ESP_ERROR_CHECK( esp_wifi_start() );
     ...
     ESP_LOGI(TAG, "Wifi: %s connected ", wifi_config.sta.ssid);
}
...
static void https_get_task(void *pvParameters)
{
    ...
    if((ret = mbedtls_ssl_set_hostname(&ssl,  parsed_url->host)) != 0)
    {
    ...
    */
    ...
    while(1) {
        ...
        */
        ...
        ESP_LOGI(TAG, "Connecting to %s:%s...", parsed_url->host, parsed_url->port_string);

        if ((ret = mbedtls_net_connect(&server_fd, parsed_url->host,
                                      parsed_url->port_string, MBEDTLS_NET_PROTO_TCP)) != 0)
        {
